...
from catagoryJokes import CATEGORY_JOKES  # 👈 Make sure this file exists
import random
...
def build_elements(facts, styles, date_str, category_pages=None):
    elements = [Spacer(1, 180), Paragraph("WHAT HAPPENED ON...", styles['cover_title']), Paragraph(date_str, styles['cover_date']), PageBreak()]
    ...
    for category, fact_list in categories.items():
        ...
        for i, fact in enumerate(fact_list):
            ...
            # Check if we need to push this to the next page
            ...
            # 💡 Only insert joke if there's still space on the current page
            if (will_exceed_word_limit or will_exceed_fact_limit) and facts_on_this_page < max_facts_per_page:
                if category in CATEGORY_JOKES:
                    joke = random.choice(CATEGORY_JOKES[category])
