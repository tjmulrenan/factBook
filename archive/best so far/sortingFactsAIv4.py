...
# Rewrites and classifies a batch of facts using OpenAI
...
def enhance_facts_with_openai(batch):
    prompt = (
        ...
        "- Speak directly to the reader using a fun intro, but vary your tone so no two facts sound the same.\n"
        "- Use simple language with no complicated words.\n"
        "- Include a specific detail about what the person/event is most famous for — like a major achievement, famous role, invention, or what made them stand out.\n"
        "- Keep it PG and educational.\n"
        "- Add a fun or surprising twist to each fact (like a wow moment, comparison, or strange detail).\n"
        "- When it fits, connect the story to something a kid might know about (pets, school, art, fairness, science, games, etc.), but don't force it.\n"
        "- Make every story feel unique — avoid repeating formats or words.\n"
