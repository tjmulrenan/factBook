...
def build_elements(facts, styles, date_str, category_pages=None):
    ...
    # Count total facts from JSON
    num_facts = len(facts)

    # Add the first page image (if it exists)
    first_image_path = os.path.join(os.getcwd(), "pictures", "first_page_image.png")
    if os.path.exists(first_image_path):
        elements.append(Spacer(1, 100))
        elements.append(RLImage(first_image_path, width=220, height=220))
    ...
    My name’s Tim Mulrenan — but you can call me TJ, or just Tim, or even “that guy from Saffron Walden who’s a bit too obsessed with facts.” I come from a little town full of charm, cobblestones, and very patient people who’ve listened to my fact rants since I was a kid.

    I’ve always loved learning fun things about random days — stuff that makes you go, “Wait… that really happened?” Big discoveries, strange events, and all kinds of trivia. So I turned that obsession into a whole series of books, each one focused on just one day!

    Whether <b>{date_str}</b> is your birthday, your pet lizard’s, the time your cousin ate 27 marshmallows in one sitting, or just a random date you thought looked lucky… this book’s made for you.

    By the end, you’ll know more about this day than most people know about an entire year. And who knows — maybe you’ll become a trivia genius just like me. Or at least win an argument in the playground or impress your gran.

    There are <b>{num_facts}</b> facts packed into these pages, and every one of them is as exciting, strange, and brilliant as the last. So let’s get this show on the road!
    ...
    for category, fact_list in list(categories.items())[:1]:
...
def generate_pdf_with_manual_toc(json_file, output_pdf="test_book.pdf"):
